...
// PDF/PPTX/DOCX/TXT are the AI-readable set (RAG chatbot, quiz generation,
// AI-assisted grading all extract text from these — TXT trivially, just a
// UTF-8 read). JPG/PNG/ZIP were added on request for non-text course
// content (e.g. a photographed handwritten answer, a code-submission
// archive) — they upload, verify, download, and list exactly like the
// others, but text extraction is intentionally not attempted for them (see
// ragEngine.extractText's dispatcher); AI features just note "no usable
// text" the same way an image-only PDF scan already does, rather than
// blocking the upload.
const ALLOWED_MATERIAL_EXTENSIONS = [".pdf", ".pptx", ".docx", ".txt", ".jpg", ".jpeg", ".png", ".zip"];
  ...
  return multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (!ALLOWED_MATERIAL_EXTENSIONS.includes(ext)) {
        const err = new Error("Only PDF, PPTX, DOCX, TXT, JPG, PNG, or ZIP files are allowed");
        err.statusCode = 400;
        return cb(err);
      }
      cb(null, true);
